...
;;;; `Tokens: <input>|<output>`.  Output tokens are additive across requests
;;;; (each request's output is fresh), so we sum the per-request increments.
;;;; Input tokens subsume the entire conversation context so far, so we track
;;;; the latest value (the high-water mark) rather than summing -- summing
;;;; would double-count prior context on every follow-up request.
;;;;
...
(defun get-token-counts (protocol)
  "Return (values input output) -- the latest input and cumulative output
token counts for PROTOCOL, both 0 when nothing has been tracked yet."
...
(defun record-request-usage (protocol request-id input-tokens output-tokens)
...
first delta counts in full.  INPUT-TOKENS reflects the full context sent with
the request, so we keep the high-water mark across all requests rather than
summing.  No-op when both values are nil/missing."
  (when (and request-id
...
(defun format-token-line (protocol &optional theme)
  "Format the footer line: `Tokens: <input>|<output>`.  Returns NIL when no
tokens have been counted yet, so the widget draws no line until there is
something to show."
  (multiple-value-bind (input output)
    ...
    (when (or (and input (plusp input))
      ...
      (let* ((in-str (humanize-token-count input))
             ...
             (body (format nil "~A|~A" in-str out-str)))
        (if theme
            (format nil "~A ~A"
